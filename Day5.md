# Day 5: JavaScript: From First Steps to Professional
This README file summarizes the JavaScript lesson on Conditionals, Map & filter and Doggos Quiz Game

## Lesson Summary
if statement lets us execute code under a certine condition.

## Exercise 1:
1. write a coditional that logs a message saying whether your first name or last name is longer
    ```javascript  
   if("haneen".length > "ibrahem".length)
      console.log("First name is longer");
    else if("ibrahem".length > "haneen".length)
...
## Exercise 3:
1. create a new array name with only the name of each girle
    ```javascript  
   const names = spices.map(spice => spice.name
    ```
2. crate s new array endInY with just the girls whose nickname ends in "y"
    ```javascript 
